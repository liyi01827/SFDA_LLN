...
def data_load(args):
    ...
    train_source_dataset = dataset(root=args.root, task=args.target.upper()[0], r=0, download=False, transform=image_train())

    num_classes = train_source_dataset.num_classes
    args.nb_classes = num_classes

    val_dataset = dataset(root=args.root, task=args.target.upper()[0], r=0, download=False, transform=image_test())
...
def train_target(args):
    ...
    weights_dir = args.source.upper()[0]
    args.name = weights_dir
    netF.load_state_dict(torch.load('~/da_weights/office/{}/source_F.pt'.format(weights_dir)))
    netB.load_state_dict(torch.load('~/da_weights/office/{}/source_B.pt'.format(weights_dir)))
    netC.load_state_dict(torch.load('~/da_weights/office/{}/source_C.pt'.format(weights_dir)))
    ...
    with open('./office31/elr.txt', 'a') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--source', default='amazon', help='source domain(s)')
    parser.add_argument('-t', '--target', default='webcam', help='target domain(s)')
    ...
    if args.all:
        task = ['amazon', 'dslr', 'webcam']
        acc_acg = []
        for my_source in task:
            task1 = {'amazon', 'dslr', 'webcam'}
            task1_rm = {my_source}
        ...
        with open('./office31/elr_all.txt', 'a') as f:
